...
void change_permissions(const fs::path &p, const std::string &modeStr) {
    ...
    if (!done) {
        // Very primitive parsing for patterns like "+x", "u+r", "a-w" etc (limited)
        // For safety and simplicity, do a toggle-based approach.
        fs::perms cur = fs::status(p, ec).permissions();
        fs::perms newp = cur;
        // This example only supports these commands: u/g/o/r/w/x with +/-.
        // We'll parse simple tokens separated by commas.  std::string token;
        for (size_t i=0;i<modeStr.size();++i) {
            char c = modeStr[i];
            token.push_back(c);
        }
        // For production code, replace this shim by a robust parser or shell out to chmod.
